"""
DIRECTOR v4 — Industry-grade content analysis:
...
def _analyze_pitch(audio_path, segments):
    ...
    Higher pitch → likely female/child. Lower → likely male/elderly.
    Returns dict: seg_id → pitch_hint ("high"/"mid"/"low")
    """
    pitch_hints = {}
    for seg in segments[:80]:
        sid = str(seg["id"])
        ...
        try:
            ...
            r = subprocess.run(cmd, capture_output=True, text=True, timeout=10)
            # Parse RMS level as rough pitch proxy
            rms_vals = []
            for line in r.stderr.split("\n"):
            ...
            if rms_vals:
                ...
                if avg_rms > -20:
                    pitch_hints[sid] = "high"   # Loud → possibly female/child
                elif avg_rms > -35:
                    pitch_hints[sid] = "mid"
                else:
                    pitch_hints[sid] = "low"    # Quiet → possibly male/elderly
            else:
                pitch_hints[sid] = "mid"
        except Exception:
            pitch_hints[sid] = "mid"
...
def detect_scenes(segments, energy_map=None):
    ...
    A scene boundary is when:
      - Gap between segments > 2.0s, OR
      - Major energy shift (high→low or low→high) + gap > 1.0s
      
    Returns list of scene dicts: {"start_seg": idx, "end_seg": idx, "segments": [...]}
    """
    if not segments:
        return []
    
    boundaries = [0]  # First segment is always a scene start
    
    for i in range(1, len(segments)):
        gap = segments[i]["start"] - segments[i-1]["end"]
        
        # Long pause = definite scene boundary
        if gap > 2.0:
            boundaries.append(i)
            continue
        
        # Medium pause + energy shift = likely scene boundary
...
CHUNK_SIZE = 30  # Analyze 30 segments at a time
OVERLAP = 5      # Overlap between chunks for continuity


def _analyze_chunk(segments, chunk_idx, total_chunks, summary_so_far, 
                   energy_map, pitch_hints, word_analysis, user_description):
    ...
    for i, s in enumerate(segments):
        ...
        # Add pitch and energy hints inline
        sid = str(s["id"])
        hints = []
...
def _smooth_speaker_map(speaker_map, segments):
    ...
    for i in range(1, len(seg_ids) - 1):
        ...
        # If sandwiched and gap is small, absorb
...
def analyze(segments, work_dir, user_description="", whisper_words=None, audio_path=None):
    ...
    1. Audio energy analysis (mood detection)
    2. Pitch analysis (speaker gender hints)
    3. Word-level pause analysis
    4. Chunked LLM analysis with overlap
    5. Speaker consistency post-processing
    6. Scene boundary detection
    """
    ...
    if len(segments) <= CHUNK_SIZE + 5:
        # Small enough for single analysis
        logger.info(f"[DIRECTOR] Single-pass analysis ({len(segments)} segments)")
        plan = _analyze_chunk(
            segments, 0, 1, "", energy_map, pitch_hints, word_analysis, user_description
        )
        combined_plan.update(plan)
    else:
        # Chunked analysis with overlap
        n_chunks = (len(segments) + CHUNK_SIZE - OVERLAP - 1) // (CHUNK_SIZE - OVERLAP)
        logger.info(f"[DIRECTOR] Chunked analysis: {n_chunks} chunks of ~{CHUNK_SIZE} segments")
        
        summary_so_far = ""
        seen_voices = set()
        
        for ci in range(n_chunks):
            start = ci * (CHUNK_SIZE - OVERLAP)
            ...
            try:
                ...
                # Merge maps (skip overlap segments — keep previous chunk's assignment)
