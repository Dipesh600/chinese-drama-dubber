...
import logging
from config import MIN_SEGMENT_DUR, MAX_SEGMENT_DUR
logger = logging.getLogger(__name__)


def merge_short_segments(segments):
    """Merge micro-segments before translation."""
    if not segments:
        return segments
    ...
    while i < len(segments):
        ...
        while i + 1 < len(segments):
            ...
            if (dur < MIN_SEGMENT_DUR or len(seg["text"].strip()) < 6) and combined <= MAX_SEGMENT_DUR and same_speaker:
                seg["text"] = seg["text"].rstrip() + " " + nxt["text"].strip()
                seg["end"] = nxt["end"]
                seg["mood"] = nxt.get("mood", seg.get("mood", "neutral"))
...
def expand_dubbed_to_subsegments(merged_segments):
    """Expand merged segments back to original sub-segments with distributed text."""
    expanded = []
